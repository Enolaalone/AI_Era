...
def get_tensor(names, countries):
    ...
    # 名字张量扩充
    name_tensors = torch.zeros(len(length), torch.max(length, dim=0)[0]).long()#embedding 输入要求long()整型
...
def train():
    ...
    for idx, (name, country_id) in enumerate(train_loader, 1):
        ...
        correct += pre.eq(country_t.view(-1)).sum().item()
        total += len(pre)
